...
def find_red_guide(arr: np.ndarray) -> tuple[int, int, int, int] | None:
    ...
    red = (
        (r > 145)
        & (r > g * 1.5)  # dinaikkan dari 1.3 → 1.5 untuk bedakan guide asli dari noise tanah
        & (r > b * 1.5)
        & (g < 195)
        & (b < 155)
    )
...
def locate_date_box(
    ...
    folder_date_fixed: float | None = None,
    ...
    # Stage 1a-format: Jika format timemark seragam, override Y
    if y_date_local is not None and folder_date_fixed is not None:
        if abs(y_date_local - folder_date_fixed) > 5:
            print(f"  Stage 1a-format: Y_date={y_date_local:.1f} → format Y={folder_date_fixed:.1f}")
            y_date_local = folder_date_fixed
    ...
    # STAGE 1b: Konsensus Tanggal Folder / Format Seragam
    y_pakai = folder_date_consensus if folder_date_consensus is not None else folder_date_fixed
    if y_date_local is None and y_pakai is not None:
        label = "Konsensus" if folder_date_consensus else "Format Seragam"
        y1 = int(y_pakai - box_h / 2)
        y2 = y1 + box_h
        x1 = int(w * 0.047)
        x2 = int(w * 0.49)
        print(f"  STAGE 1b ({label}): Y_date={y_pakai:.1f} TextBox Y={y1}-{y2}")
        return max(0, x1), max(0, y1), min(w, x2), min(h, y2)

    # STAGE 1c: Red Guide Anchor (Posisi Baru) - untuk foto dengan guide tapi OCR gagal
    # Textbox di KANAN guide, center sejajar dengan bagian ATAS guide
    # Box extends: 25% di atas center, 75% di bawah center
    guide = find_red_guide(arr)
    if guide:
        gx1, gy1, gx2, gy2 = guide
        # Hanya gunakan Stage 1c jika guide terdeteksi dengan valid
        # Posisi baru: di kanan guide, center = guide top
        new_box_h = int(h * 0.053)
        new_y_center = gy1  # Center sejajar dengan bagian atas guide
        new_y1 = int(new_y_center - new_box_h * 0.25)  # 25% di atas center
        new_y2 = int(new_y_center + new_box_h * 0.75)  # 75% di bawah center
        new_x1 = gx2 + int(w * 0.018)  # Sedikit di kanan guide
        new_x2 = min(w, new_x1 + int(w * 0.45))
        
        print(f"  STAGE 1c (Red Guide Anchor): guide_top={gy1} TextBox Y={new_y1}-{new_y2}")
...
def process_image(
    ...
    detection = locate_date_box(
        ...
        folder_date_fixed=folder_date_fixed,
    )
...
def main() -> int:
    ...
    # --- Cek format timemark seragam per folder ---
    # Jika semua foto folder punya ISO date sama, paksa Y seragam
    from PIL import Image as PIL_Image
    folder_date_fixed = {}
    if schedule_lookup and images_by_folder:
        for folder, folder_images in images_by_folder.items():
            date_parts_set = set()
            for img_path in folder_images:
                try:
                    asset_type = img_path.parent.parent.name
                    detail = img_path.parent.name
                    photo_name = img_path.name
                    key = (asset_type, detail, photo_name)
                    entry = schedule_lookup.get(key)
                    if entry:
                        iso_ts = entry[0]
                        date_parts_set.add(iso_ts[:10])
                except IndexError:
                    pass
            if len(date_parts_set) == 1 and len(folder_images) >= 2:
                fixed_y = folder_date_consensuses.get(folder)
                if fixed_y is None:
                    for img_path in folder_images:
                        arr_tmp = np.array(PIL_Image.open(img_path))
                        y = detect_date_y_center(arr_tmp, arr_tmp.shape[0], arr_tmp.shape[1])
                        if y is not None:
                            fixed_y = y
                            print(f"[FORMAT] Folder '{folder.name}': Format seragam, Y={fixed_y:.1f} (dari {img_path.name})")
                            break
                else:
                    print(f"[FORMAT] Folder '{folder.name}': Format seragam, Y={fixed_y:.1f} (konsensus)")
                if fixed_y is not None:
                    folder_date_fixed[folder] = fixed_y
    ...
    for image_path in images:
        ...
        date_fixed = folder_date_fixed.get(image_path.parent)
